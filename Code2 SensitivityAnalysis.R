...
seir <- odin({
  ...
  latent_period <- user(1.5) #latent periods; time from exposed to infectious
  infectious_period <- user(10.8) # infectious period; time stay infectious
  incubation_period <- user(3.6) # incubation period; time from exposed to clinical sign
...
TAU_MIN <- runif(REP, min = 1, max = 14)
TAU_DIFF <- runif(REP, min = 1, max = 14)

TAU_MAX <- TAU_MIN + TAU_DIFF
...
for(i in 1:REP){
  ...
  immune_v_function <- function(time, t_imm_min, t_imm_max){
  ...
  for(nw in 1:nWeek){
    p_immuned[,nw] <- immune_v_function(time + (7*(nw-1)), t_imm_min = t_imm_min, t_imm_max = t_imm_min + t_diff)
  }
  ...
  for(w in 1:maxit){
    ...
    conditionList <- c("b" = b, "r" = r, "delta_asymptomatic" = delta_asymptomatic, 
                       ...
                       "t_imm_min" = t_imm_min, "t_diff" = t_diff)
    #changing the class
...
for(i in 1:length(ALL_list)){
  ...
  summary <- data.frame("Beta" = ALL_list[[i]][[1]][1],
                        ...
                        "t_imm_min" = ALL_list[[i]][[1]][11],
                        "t_diff" = ALL_list[[i]][[1]][12],
                        "risk" = risk,
                        "prot" = prot,
                        "erad" = erad,
...
All_summary$t_imm_min <- rank(All_summary$t_imm_min)
All_summary$t_diff <- rank(All_summary$t_diff)
...
write.csv(All_summary, "Sensitivity_results_new.csv", row.names = F)
colnames(All_summary)

################################################
col1 <- c("Beta_r", "R_r", "Delta_asymp_r",  "vaccine_effectiveness_r",          
          "prev_r", "Imm_r", "pi_r", "gamma_IA2_IS_r", "gamma_IA1_R_r", "gamma_IS_R_r", "t_imm_min", "t_diff", "risk_r")
col2 <- c("Beta_r", "R_r", "Delta_asymp_r",  "vaccine_effectiveness_r",           
          "prev_r", "Imm_r", "pi_r", "gamma_IA2_IS_r", "gamma_IA1_R_r", "gamma_IS_R_r", "t_imm_min", "t_diff",  "prot_r")
col3 <- c("Beta_r", "R_r", "Delta_asymp_r",  "vaccine_effectiveness_r",          
          "prev_r", "Imm_r", "pi_r", "gamma_IA2_IS_r", "gamma_IA1_R_r", "gamma_IS_R_r", "t_imm_min", "t_diff",  "erad_r")
col4 <- c("Beta_r", "R_r", "Delta_asymp_r",  "vaccine_effectiveness_r",           
          "prev_r", "Imm_r", "pi_r", "gamma_IA2_IS_r", "gamma_IA1_R_r", "gamma_IS_R_r", "t_imm_min", "t_diff",  "weekNotSafe_r")
...
P1$hjust <- ifelse(P1$est > 0, 1.4, -0.4)

P2$col <- P2$est > 0
P2$sig <- ifelse(P2$p.value < 0.05, "*", "ns")
P2$hjust <- ifelse(P2$est > 0, 1.4, -0.4)

P3$col <- P3$est > 0
P3$sig <- ifelse(P3$p.value < 0.05, "*", "ns")
P3$hjust <- ifelse(P3$est > 0, 1.4, -0.4)

P4$col <- P4$est > 0
P4$sig <- ifelse(P4$p.value < 0.05, "*", "ns")
P4$hjust <- ifelse(P4$est > 0, 1.4, -0.4)


P1$param <- factor(P1$param, levels = c("Beta_r", "R_r", "Delta_asymp_r",  "vaccine_effectiveness_r", "t_imm_min", "t_diff",         
                                        "prev_r", "Imm_r", "pi_r", "gamma_IA2_IS_r", "gamma_IA1_R_r", "gamma_IS_R_r"))
P2$param <- factor(P2$param, levels = c("Beta_r", "R_r", "Delta_asymp_r",  "vaccine_effectiveness_r",  "t_imm_min", "t_diff",        
                                        "prev_r", "Imm_r", "pi_r", "gamma_IA2_IS_r", "gamma_IA1_R_r", "gamma_IS_R_r"))
P3$param <- factor(P3$param, levels = c("Beta_r", "R_r", "Delta_asymp_r",  "vaccine_effectiveness_r", "t_imm_min", "t_diff",       
                                        "prev_r", "Imm_r", "pi_r", "gamma_IA2_IS_r", "gamma_IA1_R_r", "gamma_IS_R_r"))
P4$param <- factor(P4$param, levels = c("Beta_r", "R_r", "Delta_asymp_r",  "vaccine_effectiveness_r",  "t_imm_min", "t_diff",        
                                        "prev_r", "Imm_r", "pi_r", "gamma_IA2_IS_r", "gamma_IA1_R_r", "gamma_IS_R_r"))


G1 <- ggplot(data = P1, aes(x = param, y = est, fill = col, label = sig, hjust = hjust)) + geom_bar(stat = "identity") +
  theme_bw() +
  coord_flip() +
  ylim(-1,1) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  scale_x_discrete(labels = c(expression(beta), expression("r"), expression(delta), expression("e"["v"]),
                              expression(tau["min"]), expression(tau["max"] - tau["min"]),
...
G2 <- ggplot(data = P2, aes(x = param, y = est, fill = col, label = sig, hjust = hjust)) + geom_bar(stat = "identity") +
  theme_bw() +
  coord_flip() +
  ylim(-1,1) +
  geom_hline(yintercept = 0, linetype = "dashed") +
...
G3 <- ggplot(data = P3, aes(x = param, y = est, fill = col, label = sig, hjust = hjust)) + geom_bar(stat = "identity") +
  theme_bw() +
  coord_flip() +
  ylim(-1,1) +
  geom_hline(yintercept = 0, linetype = "dashed") +
...
G4 <- ggplot(data = P4, aes(x = param, y = est, fill = col, label = sig, hjust = hjust)) + geom_bar(stat = "identity") +
  theme_bw() +
  coord_flip() +
  ylim(-1,1) +
  geom_hline(yintercept = 0, linetype = "dashed") +
