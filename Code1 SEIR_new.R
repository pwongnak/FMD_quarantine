...
Import_cattle <- function(Imm, prev){
  ...
  cattleE <- rbinom(1, size = cattleInf, prob =  0.12) #3.6/(3.6+4.4) 1.5/(1.5+10.8)
  cattleI <- cattleInf - cattleE
  cattleIA1 <- rbinom(1, size = cattleI, prob = 1-pi)
  cattleIs <- rbinom(1, size = cattleI - cattleIA1, prob = 0.19) #(5.9-3.6)/4.4.      (3.6-1.5)/10.8
  ...
  for(Imm in ImmList){
    for(r in RList){
      for(vaccine_effectiveness in vaccine_effectivenessList){
        ...
        write.csv(marketList, paste0("mainResults_2023_2/marketList_Imm_", Imm, "_r_", r, "_vac_", vaccine_effectiveness, "_", nWeek, "w.csv"))
        write.csv(outputALL, paste0("mainResults_2023_2/outputALL_Imm_", Imm, "_r_",  r, "_vac_", vaccine_effectiveness, "_", nWeek, "w.csv"))
      }
    }
  }
}
